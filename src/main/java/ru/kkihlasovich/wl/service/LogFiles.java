 ...
 */
   ...
   class LogFiles {
     ...
     private final static org.apache.log4j.Logger logger = LogManager.getLogger(LogFiles.class);
     ...
     private List<Log> getLogFiles(String zone, String word, XMLGregorianCalendar dateFrom, XMLGregorianCalendar dateTo) {
        ...
        OneFileLogs oneFileLogs = new OneFileLogs();
        for (File item : dir.listFiles()) {
            if (isLogFile(item)) {
                zoneLogs.addAll(oneFileLogs.getFileLogs(item.getAbsolutePath(), word, dateFrom, dateTo));
            }
        }
        return zoneLogs;
    }
